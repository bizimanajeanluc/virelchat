...
const targetAdminEmail = 'bizimananajeanluc73@gmail.com';
const targetAdminPhone = '0723223653';
...
io.on('connection', (socket) => {
  ...
  try {
    ...
    socket.on('voice_recording', (data) => {
      io.to(`user:${data.recipientId}`).emit('voice_recording', { senderId: userId, isRecording: data.isRecording });
    });
